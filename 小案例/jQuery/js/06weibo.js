...
$(function(){
	// 一、页面加载完成后，初始化 表情 的div
	var $faceDiv = $("<div class='faceDiv'></div>"); // 创建div
	...
	// 二、页面加载完成后，初始化 朋友 的div
	var $atSomeList = $("<ul class='atSomeList'></ul>");
	for( var name in friendsList ) {
		var $li = $("<li><a href='#'>@"+friendsList[name]+"</a></li>");
		$atSomeList.append($li);
	...
	$atSomeList.hide().css({
		left: $( ".atSome" ).offset().left + "px",
		top: $(".atSome").offset().top  + "px"
	});
	$atSomeList.find("a").click(function(){
