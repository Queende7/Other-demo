...
function initLeft(){
    ...
    var setTransform = function(t){
        childDom.style.transform = "translateY("+t+"px)";
        childDom.style.webkitTransform = "translateY("+t+"px)";
    }
   ...
   /*晃动的范围*/
    ...
    childDom.addEventListener('touchend',function(e){
        console.log(2);
        //当超过了0的时候就让子容器弹回去(上面满足吸附的条件)
        if((currY - moveY) >= 0){
             addTransition();
             setTransform(0);
        }
        //当超过了最大上滑动距离的时候就让子容器弹回去(下面满足吸附的条件)
        else if((currY - moveY) <= -(childH - parentH)){
            addTransition();
            setTransform(-(childH - parentH));
        }else{   //滑动结束之后记录下当前的translateY的值(正常情况)
            currY = currY - moveY;
        }
        ...
        if(moveY == 0 && endTime-startTime < 200){
            var target = e.target.parentNode;// 当前点击的 li
            //清除class，给点击的元素加上now
